...
- [ ] Integrar las fucionalidades de las tareas 1 y 2. WiFi y BLE. Esto quiere decir que un ESP32 deber ser capaz de funcionar en ambos modos, con todas las funcionalidades de ambas tareas.
- [ ] El modo de funcionamiento de las ESP32 debe ser controlado por el estado de la base de datos (Como fue mencionado en las tareas anteriores). Esto sera crucial para esta tarea.
- [ ] Los cambios en la base de datos deberan ser reflejados en el ESP32 y en funcionamiento del sistema
- [ ] La base de datos debe ser accedida desde python unicamenta (no se permitira modificarla directamente usando adminer o pgadmin)
- [ ] Deben creare una API HTTP que permita (Se dara una plantilla utilizando FastAPI):
  - `Acceder a la configuracion actual`
  - `Modifcar la configuracion actual`
  - `Acceder a los dattos generados por las ESP32`

- [ ] Una interfaz web la cual permita (Se dara una plantilla usando React):
  - `Visualizar los datos generados por las ESP32`: Para esto deben generar graficos que se actualicen en tiempo real, no se permitira mostrar datos en una tabla.
  - `Modificar la configuracion actual`


## Funcionamiento

Todo debe de funcionar segun la configuracion de la base de datos, tendremos los 2 medios de comunicacion:

### BLE:

- Protocolos de datos: 0, 1, 2 y 3
- Transport Layer: BLE Continuo y BLE Discontinuo

### WiFi:

- Protocolos de datos: 0, 1, 2, 3, 4
- Transport Layer: TCP y UDP

Se pide que segun los cambios en la base de datos puedan cambiar tanto el medio de comunicacion (BLE o WiFi) como el protocolo de datos y el transport layer.

Por simplicidad se tendra un estado de inico (Le diremos estado a la configuracion de medio, protocolo y transport layer), se debe de iniciar el funcionamiento utilzando BLE Continuo y el protocolo 0. Luego se podra cambiar el estado a traves de la API HTTP)

Luego se debe de poder cambiar desde cualquier estado a cualquier otro estado, por ejemplo:
...
1. Se inicia el sistema con BLE Continuo y protocolo 0 y se realiza una conexion entre la ESP32 y la Raspberry Pi. Se envian los datos segun el protocolo 0.

2. En la interfaz web se pueden ver los datos adquiridos por la ESP32 y se puede cambiar el estado de la base de datos a TCP y protocolo 4. (Esto realiza una llamada a la API HTTP, la cual interactua con la base de datos)

3. En el siguiente intercambio de datos entre la ESP32 y la Raspberry Pi, su codigo en python detecta el cammbio en la base de datos y envia la configuracion actualizada a la ESP32.

4. La ESP32 recibe la configuracion actualizada y cambia su medio de comunicacion a TCP y su protocolo de datos a 4. Luego envia los datos segun el protocolo 4.

5. Se ven realizados los cambios en la interfaz web y se vuelve al paso 2 para realizar otro cambio.

Aqui es importante notar que la ESP no puede trabajar con WiFi y BLE al mismo tiempo, por lo que se debe de cerrar la conexion BLE para poder trabajar con WiFi y viceversa. Para esto les puede ser util
simplemente reiniciar la ESP32 cuando se cambie el estado de la base de datos y inicializarla con el nuevo estado. Con esto surge un problema nuevo 'y donde queda la informacion de que protocolo usar despues de reiniciar la ESP32?'.

Para esto les sera util el uso de la memoria no volatil (En el apunte se explica como utilizarla), de esta forma cuando a la ESP32 le llegue una configuracion nueva, la guardara en la memoria no volatil y cuando se reinicie la ESP32, esta leera la configuracion guardada en la memoria no volatil y se inicializara con esta.
...
Dentro del README.md de la plantilla se encuentra la informacion necesaria para poder utilizarla.
